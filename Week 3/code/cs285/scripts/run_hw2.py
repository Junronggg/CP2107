...
def run_training_loop(args):
    ...
    for itr in range(args.n_iter):
        ...
        trajs_dict = {
            'obs': [traj['observation'] for traj in trajs],
            'acs': [traj['action'] for traj in trajs],
            'rewards': [traj['reward'] for traj in trajs],
            'terminals': [traj['terminal'] for traj in trajs],
        }
        # print("Keys in trajs_dict:", trajs_dict.keys())  # ← Debug print here


        # TODO: train the agent using the sampled trajectories and the agent's update function
        train_info = agent.update(
            obs=trajs_dict['obs'],
            actions=trajs_dict['acs'],
            rewards=trajs_dict['rewards'],
            terminals=trajs_dict['terminals'],
        )
